...
def make_subtitle_clip(text: str, talker: str, config):
    ...
    subtitle_clip = TextClip(
        text=text,
        font=resource_path(config.SUBTITLE_FONT),  # フォントも絶対パス化
...
def make_voice_clip(text: str, speaker: int = 1, speed: float = 1.0, silence_duration: float = 1.0) -> AudioClip | None:
    ...
    # data/temp_data フォルダを exe と同じ階層から参照
    temp_data_dir = resource_path(os.path.join("data", "temp_data"))
    os.makedirs(temp_data_dir, exist_ok=True)  # 無ければ作成

    filepath = os.path.join(temp_data_dir, f"voice_{safe_filename}_{speed}_{speaker}.wav")
